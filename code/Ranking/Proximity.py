...
def get_proximity_score(query, docID: int):
    score = 0
    body_positions_dict = {}
    title_positions_dict = {}

    query_terms = query.split()

    for query_term in query_terms:
        body_positions = inverted_index.get(query_term).docMap[docID].body_positions
        title_positions = inverted_index.get(query_term).docMap[docID].title_positions


        body_positions_dict[query_term] = body_positions
        title_positions_dict[query_term] = title_positions



    pairs: list = _get_query_pairs(query_terms)

    for pair in pairs:
        t1_body_positions = body_positions_dict.get(pair[0], [])
        t2_body_positions = body_positions_dict.get(pair[1], [])

        t1_title_positions = title_positions_dict.get(pair[0], [])
        t2_title_positions = title_positions_dict.get(pair[1], [])
